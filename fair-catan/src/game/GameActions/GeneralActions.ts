  ...
  for (let vertex = 0; vertex < Board.NUM_VERTICES; ++vertex) {
    ...
    if (owner != undefined) {
      for (const tile of gameState.board.GetTilesOfVertex(vertex)) {
        ...
        owner.resources[resource] += ProbabilityOfRollingValue(
          gameState.board.tileValues[tile],
        );
        result.push(
          `${owner.name} gains ${ProbabilityOfRollingValue(gameState.board.tileValues[tile])} of ${resource}`,
        );
      }
      continue;
    }
    ...
    if (owner != undefined) {
      for (const tile of gameState.board.GetTilesOfVertex(vertex)) {
        ...
        owner.resources[resource] +=
          2 * ProbabilityOfRollingValue(gameState.board.tileValues[tile]);
        result.push(
          `${owner.name} gains ${2 * ProbabilityOfRollingValue(gameState.board.tileValues[tile])} of ${resource}`,
        );
      }
    }
  }
...
export function BuyDevCard(gameState: GameState, player: Player): string {
  if (gameState.currentPlayer != player)
    return "You can only buy development cards during your turn";
  if (
...
export function PlayDevCard(
  ...
  if (gameState.currentPlayer != player)
    return "You can only play development cards during your turn";
  if (gameState.devCardPlayedThisTurn)
    return "You can only play one development card per turn";
  if (cardToPlay == DevCard.VictoryPoint)
    return "You can't play victory cards, they will automatically be revealed once you reach 10 points";
  const numOfCardsPlayerOwns = player.playableDevCards.filter(
    (x) => x == cardToPlay,
  ).length;
  if (numOfCardsPlayerOwns <= 0)
    return `You do not have a ${cardToPlay} development card`;
  const numOfCardsBoughtThisTurn = gameState.cardsBoughtThisTurn.filter(
    (x) => x == cardToPlay,
  ).length;
  if (numOfCardsBoughtThisTurn >= numOfCardsPlayerOwns)
    return "You cannot play development cards on the same turn you bought them";

  switch (cardToPlay) {
    case DevCard.Knight:
      PlayKnightCard(gameState, player, args);
      break;

    case DevCard.RoadBuilding:
      if (args.length == 0 || args.length > 2)
        return `Provide two arguments for the two road places you want to build`;
    ...
    case DevCard.YearOfPlenty: {
      if (args.length != 2)
        return `You need to select two resources to take, 0=Brick, 1=Lumber, 2=Ore, 3=Grain, 4=Wool`;
      const resource1 = MapNumberToResource(args[0]);
      if (resource1 == undefined)
        return `You need to name a resource, 0=Brick, 1=Lumber, 2=Ore, 3=Grain, 4=Wool`;
...
function PlayKnightCard(gameState: GameState, player: Player, args: number[]) {
  if (args.length > 2) {
    return `You can have a maximum of two arguments when playing a knight card: The tile you move the robber to, and (if applicable) the opponent to steal from`;
  } else if (args.length < 1) {
    return `You need at least one argument to play a knight card: the tile you move the robber to`;
  }
  const tile = args[0];
  if (tile == gameState.board.robber)
    return "You must move the robber to a different tile when playing the knight card";
  if (tile < 0 || tile >= gameState.board.tileResources.length)
    return `Tiles are numbered 0-${gameState.board.tileResources.length - 1}, no tile with number ${tile} exists`;
  if (args.length == 2) {
    const victimVertex = args[1];
    if (!gameState.board.GetVerticesOfTile(tile).has(victimVertex))
      return `The vertex ${victimVertex} is not on the tile ${tile}, you must steal from a player on that tile`;
    let victim = gameState.board.settlements[victimVertex];
    if (victim == undefined) victim = gameState.board.cities[victimVertex];
    if (victim == undefined)
      return `There is no player on vertex ${victim} to steal from`;
    ...
    const resourcesCanSteal: Resource[] = [];
    ...
    for (const vertex of gameState.board.GetVerticesOfTile(tile)) {
      if (
        gameState.board.settlements[vertex] != undefined &&
        gameState.board.settlements[vertex] != player
      )
        return `You must steal from an opponent built on the tile you are placing the robber on`;
      if (
        gameState.board.cities[vertex] != undefined &&
        gameState.board.cities[vertex] != player
      )
        return `You must steal from an opponent built on the tile you are placing the robber on`;
    }
...
export function UseHarbor(
  ...
  harborVertexLocation: number,
): string {
  // check that it is the players turn
  if (gameState.currentPlayer != player)
    return "You can only trade at harbors during your turn";

  // get the harbor at the specificed location
  const harbor: Harbor | undefined = gameState.board.harbors.find(
    (harbor) =>
      harbor.locations[0] == harborVertexLocation ||
      harbor.locations[1] == harborVertexLocation,
  );
  if (harbor == undefined)
    return `There is no harbor located at vertex ${harborVertexLocation}`;

  // check that they have a city or settlement at the specified location
  if (
    gameState.board.cities[harborVertexLocation] != player &&
    gameState.board.settlements[harborVertexLocation] != player
  )
    return `You do not have a city or settlement at vertex ${harborVertexLocation}`;

  // make sure the player has enough resources to give of the specified type
  if (harbor.harborType == undefined) {
    if (player.resources[giveResource] < 3)
      return `You need at least 3 ${giveResource} to trade, you only have ${player.resources[giveResource]}`;
    ...
    if (player.resources[giveResource] < 2)
      return `You need at least 2 ${giveResource} to trade, you only have ${player.resources[giveResource]}`;
...
export function BankExchange(
  ...
  if (player.resources[giveResource] < 4)
    return `You must have at least 4 ${giveResource} to exchange with the bank, you only have ${player.resources[giveResource]}`;
