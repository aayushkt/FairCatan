...
export class Trade {
  ...
  IsExactSame(otherTrade: Trade): boolean {
    return (
      this.initiator == otherTrade.initiator &&
  ...
  IsMirrorTrade(otherTrade: Trade): boolean {
    return (
      this.initiator == otherTrade.recipient &&
  ...
  IsPossibleTrade(): boolean {
    return (
      this.initiator.resources[this.giveResource] >= this.giveAmount &&
      this.recipient.resources[this.wantResource] >= this.wantAmount
    );
  }
}
...
export function PlaceOffer(
  ...
  if (player != trade.initiator)
    return `You must be the initiator to place an offer`;
  if (!trade.IsPossibleTrade())
    return `This trade is not possible as one or more players don't have the required resources`;
  for (const existingTrade of gameState.activeTradeOffers) {
    if (existingTrade.IsExactSame(trade))
      return `That trade is already on the table`;
    if (existingTrade.IsMirrorTrade(trade))
      return AcceptOffer(gameState, player, existingTrade);
  }
...
export function CancelOffer(
  ...
  if (player != trade.initiator)
    return `You can only cancel trades you initiated. If you are the recipient, use DeclineOffer instead`;
  gameState.activeTradeOffers = gameState.activeTradeOffers.filter(
    (offer) => !offer.IsExactSame(trade),
  );
...
export function AcceptOffer(
  ...
  if (player != trade.recipient)
    return `You must be the recipient to accept an offer`;
  const lenBeforeRemoving = gameState.activeTradeOffers.length;
  gameState.activeTradeOffers = gameState.activeTradeOffers.filter(
    (offer) => !offer.IsExactSame(trade),
  );
  if (lenBeforeRemoving == gameState.activeTradeOffers.length)
    return `No matching placed trade offer was found to accept`;
...
export function DeclineOffer(
  ...
  if (player != trade.recipient)
    return `You can only decline offers if you are the recipient. Did you mean to CancelOffer() instead?`;
  gameState.activeTradeOffers = gameState.activeTradeOffers.filter(
    (offer) => !offer.IsExactSame(trade),
  );
